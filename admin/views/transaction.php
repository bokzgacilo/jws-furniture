...
<?php
  ...
  $transaction = $conn -> query("SELECT * FROM transactions");
  $total_transactions = $transaction -> num_rows;

  $mode = $conn -> query("SELECT mode FROM transactions");
  ...
  $total_earnings = $conn -> query("SELECT SUM(amount) FROM transactions");

  $total_earnings = $total_earnings -> fetch_assoc();
  $total_earnings = number_format($total_earnings['SUM(amount)'], 2, '.', ',');

  $recent_transactions = $conn -> query("SELECT * FROM transactions ORDER BY date_created DESC LIMIT 7");
?>
...
var myChart = new Chart(ctx, {
    type: 'bar',
    data: {
      labels: ['GCash', 'PayPal', 'Maya'],
      datasets: [{
        label: 'Number of Transactions',
        data: [<?php echo "$GCASH, $PAYPAL, $MAYA"; ?>],
        backgroundColor: [
          'rgba(255, 99, 132, 0.2)',
          'rgba(54, 162, 235, 0.2)',
          'rgba(255, 206, 86, 0.2)'
